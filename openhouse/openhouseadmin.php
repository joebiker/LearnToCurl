...
if( isset($_POST['type']) && (strlen($_POST['type']) > 4) ) {
	...
	if( $_POST['type'] == "newopenhouse" ) {  // create new record
		$result = mysql_query("insert into openhouse_dates(event_date, event_name, max_guests, comments, event_type) values('".$_POST['newdate']."', '".$_POST['newname']."', ".$_POST['newmax'].", '".$_POST['newcomments']."', upper('".$_POST['newtype']."'))");
		if ($result) {
...
if(isset($_GET['view']))
$result = mysql_query("select EVENT_NAME, EVENT_DATE, MAX_GUESTS, ID, COMMENTS from openhouse_dates where id = ".$_GET['view']." order by EVENT_DATE ASC", $db_conn);
...
if($result && isset($_GET['view']) ) { //query was a success
	...
	while ($row = mysql_fetch_array($result, MYSQL_BOTH)) {
		...
		$resultdata = mysql_query("select group_name, group_adults, group_juniors, email, paid_dollars, paid_type, confirmation, attended, waiver, user_refer, learn_refer, reg_refer from openhouse, openhouse_dates where id = openhouse_id AND id = $row[3] order by EVENT_DATE ASC, group_name", $db_conn);
...
$result = mysql_query("select EVENT_NAME, EVENT_DATE, MAX_GUESTS, ID, COMMENTS, EVENT_TYPE from openhouse_dates where EVENT_DATE >= now() order by EVENT_DATE asc", $db_conn);
...
$result = mysql_query("select EVENT_NAME, EVENT_DATE, MAX_GUESTS, ID, COMMENTS, EVENT_TYPE from openhouse_dates where EVENT_DATE < now() order by EVENT_DATE desc limit 30", $db_conn);
if($result) { //query was a success
	include "openhouse_list.php";
}
